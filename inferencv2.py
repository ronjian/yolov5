...
origin_height = 960
# image_width = 672
# image_height = int(image_width / origin_width * origin_height) # 504
# input_height = 512
# pad_top = (input_height - image_height) // 2 #4
# pad_bottom = input_height - image_height - pad_top #4
resize_w = 640
resize_h = 480
pad_top = 16
pad_bottom = 16
...
if __name__ == "__main__":
    ...
    for imname in  os.listdir(val_dir):
        ...
        if imname.endswith('.jpeg'):
            ...
            input_img = cv2.resize(origin_img, (resize_w, resize_h), interpolation=cv2.INTER_AREA)
            ...
            assert input_img.shape == (input_h, input_w, 3)
            input_img = input_img[:, :, ::-1].transpose(2, 0, 1)  
            # input_img = np.ascontiguousarray(input_img)
            input_img = np.expand_dims(input_img, 0)
            input_img = input_img / 255.
            np.save("./assets/output_rk/input.npy", input_img)
            # input_img = np.load("./assets/output_torch/input.npy")
            ...
            for box in R:
                ...
                score = round(score, 4)
                x0 = int(clip((c_x - b_w / 2. - pad_left) / resize_w) * origin_width)
                y0 = int(clip((c_y - b_h / 2. - pad_top) / resize_h) * origin_height)
                x1 = int(clip((c_x + b_w / 2. - pad_left) / resize_w) * origin_width)
                y1 = int(clip((c_y + b_h / 2. - pad_top) / resize_h) * origin_height)
