...
model = Model("./models/yolov5s.inconv-half.yaml", nc=29).to(device)
# ckpt = torch.load("./weights/best_yolov5s_robo_inconv.pt", map_location=device)  # load checkpoint
# ckpt['model'] = {k: v for k, v in ckpt['model'].float().state_dict().items()
#                     if model.state_dict()[k].shape == v.shape}  # to FP32, filter
# model.load_state_dict(ckpt['model'], strict=True)
...
model(torch.Tensor(1,3,256,336))
...
trace_model = torch.jit.trace(model, (torch.Tensor(1,3,256,336), ))
trace_model.save('./weights/best_yolov5s_robo_inconv_jit_v2_half.pt')
